...
public class LoginActivity extends AppCompatActivity{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        currContext = LoginActivity.this;
        mAuth = FirebaseAuth.getInstance();
        initiateForCMLogin(true , currContext);


        // Button Event Listeners
        findViewById(R.id.go_to_register).setOnClickListener(v -> {
            startActivityIfNeeded(new Intent(getApplicationContext() , RegisterActivity.class) , 0);
        });

        findViewById(R.id.googleButtonForLogin).setOnClickListener(v -> {
            registerWithGoogleUsingCM();
        });

        findViewById(R.id.login_button).setOnClickListener(v -> {
            EditText pf =  findViewById(R.id.login_password);
            EditText ef =  findViewById(R.id.login_email);
            String password = String.valueOf(pf.getText());
            String email = String.valueOf(ef.getText());
    ...
    private void loginUsingPassword(String email ,String password){
        mAuth.signInWithEmailAndPassword(email , password).addOnCompleteListener(task -> {
            if (task.isSuccessful()) {
                FirebaseUser user = mAuth.getCurrentUser();
                if(user!=null){
                    Toast t= DynamicToast.makeSuccess(getApplicationContext(), "Login Successful:"+user.getDisplayName());
                    t.setGravity(Gravity.TOP,0 , 0 );
                    t.show();
                    goToMainActivity();
                }
            } else {
                Log.w(TAG, "loginUsingPassword:failure", task.getException());
    ...
    public void handleCMRegister(GetCredentialResponse result) {
        ...
        if (credential instanceof PublicKeyCredential) {
            ...
            if (GoogleIdTokenCredential.TYPE_GOOGLE_ID_TOKEN_CREDENTIAL.equals(credential.getType())) {
                ...
                googleAuthUsingFirebase(googleIdTokenCredential.getIdToken());
    ...
    public void googleAuthUsingFirebase(String idToken){
        ...
        mAuth.signInWithCredential(firebaseCredential)
                .addOnCompleteListener(this, task -> {
                    if (task.isSuccessful()) {
                        FirebaseUser user = mAuth.getCurrentUser();
                        if(user != null){
                            Toast t= DynamicToast.makeSuccess(getApplicationContext(), "Login Successful:"+user.getEmail());
                            t.setGravity(Gravity.TOP,0 , 0 );
                            t.show();
                            goToMainActivity();
                        }else{
